...
export function GraphView({ data, selectedNode, theme = 'normal', onNodeClick, onLinkClick }: GraphViewProps) {
  ...
  useEffect(() => {
    if (fgRef.current && rootNode) {
      ...
      fgRef.current.d3Force('radial', d3.forceRadial((d: any) => {
        if (d.id === rootNode.id) {
  ...
  const paintNode = useCallback((node: GraphNode, ctx: CanvasRenderingContext2D, globalScale: number) => {
    ...
    if (theme === 'normal') {
      // Normal: All nodes are white
      color = '#FFFFFF';
      strokeColor = '#3f3f46'; // Zinc-700 for subtle border
    } else if (theme === 'monochrome') {
      // Monochrome: Root is Zinc-100, others are Zinc-500
      color = isRoot ? '#F4F4F5' : '#52525B';
      strokeColor = '#09090b';
    } else {
      // Classic: Use original group colors
      const groupColors = ['#3b82f6', '#ef4444', '#10b981', '#f59e0b', '#8b5cf6', '#ec4899', '#06b6d4', '#84cc16', '#f97316', '#6366f1'];
      ...
      <ForceGraph2D
        ...
        linkColor={(link: GraphLink) => {
          ...
          if (sourceId === selectedNode.id || targetId === selectedNode.id) {
            ...
            return theme === 'monochrome' ? '#a1a1aa' : '#6366f1'; // Zinc-400 for monochrome, Indigo-500 otherwise
          }
